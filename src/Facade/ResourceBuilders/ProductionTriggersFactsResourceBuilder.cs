namespace Linn.Production.Facade.ResourceBuilders
{
    ...
    {
        private readonly ProductionBackOrderResourceBuilder productionBackOrderResourceBuilder =
            new ProductionBackOrderResourceBuilder();

        private readonly ProductionTriggerAssemblyResourceBuilder productionTriggerAssemblyResourceBuilder =
            new ProductionTriggerAssemblyResourceBuilder();

        private readonly WorksOrderResourceBuilder worksOrderResourceBuilder;

        public ProductionTriggersFactsResourceBuilder(IWorksOrderMessageService worksOrderMessageService)
        {
                           ...
                           ReportResults = new ProductionTriggerFactsResource
                                               {
                                                   ...
                                                   EarliestRequestedDate =
                                                       facts.Trigger.EarliestRequestedDate.ToString(),
                                                   ...
                                                   ReqtForInternalAndTriggerLevelBT =
                                                       facts.Trigger.ReqtForInternalAndTriggerLevelBT,
                                                   ReqtForInternalCustomersBI =
                                                       facts.Trigger.ReqtForInternalCustomersBI,
                                                   ReqtForInternalCustomersBIDays =
                                                       facts.Trigger.ReqtForInternalCustomersBIDays,
                                                   ReqtForInternalCustomersGBI =
                                                       facts.Trigger.ReqtForInternalCustomersGBI,
                                                   ReqtForInternalTriggerBTDays =
                                                       facts.Trigger.ReqtForInternalTriggerBTDays,
                                                   ...
                                                   StockAvailableShortNowBackOrdered =
                                                       facts.Trigger.StockAvailableShortNowBackOrdered,
                                                   StockReqtPercNt = facts.Trigger.StockReqtPercNt,
                                                   Story = facts.Trigger.Story,
                                                   OutstandingWorksOrders =
                                                       facts.OutstandingWorksOrders.Select(
                                                           w => this.worksOrderResourceBuilder.Build(w)),
                                                   ProductionBackOrders =
                                                       facts.OutstandingSalesOrders.Select(
                                                           o => this.productionBackOrderResourceBuilder.Build(o)),
                                                   WhereUsedAssemblies = facts.WhereUsedAssemblies.Select(
                                                       a => this.productionTriggerAssemblyResourceBuilder.BuildAssembly(
                                                           a))
                                               }
                       };
        }
