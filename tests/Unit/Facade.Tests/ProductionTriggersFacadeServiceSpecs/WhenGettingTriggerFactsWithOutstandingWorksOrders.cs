namespace Linn.Production.Facade.Tests.ProductionTriggersFacadeServiceSpecs
{
    ...
    public class WhenGettingTriggerFactsWithOutstandingWorksOrders : ContextBase
    {
        ...
        public void SetUp()
        {
            var trigger = new ProductionTrigger { PartNumber = "SERIES K", Description = "A serious product", Priority = "1", QtyBeingBuilt = 1 };
            ...
            {
                new WorksOrder()
                    {OrderNumber = 800000, Quantity = 1, QuantityBuilt = 0, DateRaised = new DateTime(2019, 1, 1)}
            };

            this.WorksOrderRepository.FilterBy(Arg.Any < Expression < Func<WorksOrder, bool>>>()).Returns(worksOrders.AsQueryable());
